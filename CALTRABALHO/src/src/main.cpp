...
int main() {
	...
	while (!quit) {
		...
		if (i == 0) {
			...
			cout << "Id of the Vehicle %of the battery and %/m";
			...
			if (add) {
				...
				cout << "Created a Vehicle with ID " << a.back().getId() << "  ,Battery " << a.back().getBattery() << " "
						<< " and Consume " << a.back().getConsume() << endl;
			}
		} else if (i == 2) {
			bool add = true;
			int id, velocity, z;
			...
			if (add) {
				...
				cout << "The Point "<< r.back().getId() << " is now a Refuel Station with Rechargement Velocity " << r.back().getVelocity()
						<< " and altitude " << myGraph.getVertex(id)->getZ() << endl;
			}
		}
		if (i == 3) {
			int id;
			...
			if (id != a[j].getId())
				...
				if (add) {
					...
					cout << "Now the Vehicle with ID " << a[j].getId() << " has battery " << a[j].getBattery() << " and consume " << a[j].getConsume() << endl;
				}
			}
		}
		if (i == 4) {
			...
			if (id != r[j].getId())
				...
				int velocity, z;
				bool add = true;
				cout << "Rechargement Velocity and altitude" << endl;
				cin >> velocity >> z;
				if (velocity != 1 && velocity != 2 && velocity != 3) {
				...
				if (add) {
					...
					myGraph.getVertex(r[j].getId())->setZ(z);
					cout << "Now the Refuel Station with ID " << r[j].getId() << " has rechargement velocity " << r[j].getVelocity() << " and altitude " << myGraph.getVertex(r[j].getId())->getZ() << endl;
			...
			if (id != a[j].getId())
				...
				a.erase(a.begin() + j);
				cout << "Removed the vehicle with id " << id << endl;
			}
		}
		if (i == 6) {
			int id;
			...
			if (id != r[j].getId())
				...
				cout << "The  vertex with id " << id << " isn't anymore  a Refuel Station" << endl;
